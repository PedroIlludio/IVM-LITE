...
import { Loader2, Menu, Search, Home, Play, Square, Camera, Moon, SunMedium } from "lucide-react";
...
export default function IvmViewPage() {
  ...
  */
  ...
  /**
   * A chave do Google só é obrigatória quando a base do mundo é a captura dele.
   * Um projeto com mapa 3D próprio roda sem chave nenhuma — e travar a vitrine
   * numa exigência que ela não tem seria negar o motivo de o mapa existir.
   */
  const mapa3d = project?.data.config.mapa3d;
  const bloqueadoSemChave = semChave && !mapa3d;

  const etapaCarregamento = !project
    ? "Abrindo o empreendimento..."
    : !ready
      ? (mapa3d ? "Carregando o mapa 3D..." : "Carregando a fotogrametria...")
  ...
  return (
    ...
    >
      ...
      {apiKey !== null && (apiKey || mapa3d) && building && (
        <Scene3D
          ref={sceneRef}
          apiKey={apiKey}
          mapa3d={mapa3d ?? null}
      ...
      */}
      {carregando && !tilesError && !bloqueadoSemChave && (
