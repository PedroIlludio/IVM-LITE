...
type Categoria = "unidades" | "lazer" | "entorno";
...
function EmpreendimentoDetail({
   ...
   * Vista do painel.
   *
   * `resumo` e o que abre: capa, nome, sobre e destaques -- o que convence
   * alguem a continuar olhando. Todo o resto (ficha, lazer, entorno, autores)
   * virou CATEGORIA, alcancavel pela gaveta. Antes tudo isso vinha empilhado
   * numa rolagem unica de quase dois metros, e o que estava embaixo nunca era
   * visto.
   */
  const [vista, setVista] = useState<"resumo" | "menu" | Categoria>("resumo");
  ...
  /**
   * Ficha tecnica. Vive numa constante porque e renderizada no RESUMO (os
   * numeros duros sao o que se procura logo depois do pitch), e nao mais
   * dentro de uma categoria da gaveta.
   */
  const FICHA_TECNICA = (
    <>
          {(emp.terreno !== "-" || emp.torres !== "-" || emp.peDireito !== "-" || emp.unidades || emp.elevadores) && (
            <div>
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-300">
      {/*
        Cabeçalho do painel: os traços da gaveta à esquerda, a marca ao lado.

        O nome aparecia DUAS vezes — aqui (o `MarcaDoProjeto` cai no nome quando
        o projeto não tem logo) e no título logo abaixo da capa. Agora este
        cabeçalho só mostra o LOGO; sem logo ele fica só com o botão, e quem
        nomeia o empreendimento é o título, uma vez só.
      */}
      <div className="flex shrink-0 items-center gap-2.5 border-b border-[var(--v-line)] px-3 py-3">
        <button
          onClick={() => setVista(vista === "menu" ? "resumo" : "menu")}
          data-testid="btn-menu-gaveta"
          title={vista === "menu" ? "Fechar" : "Explorar o empreendimento"}
          className="shrink-0 rounded-[8px] p-1.5 text-[var(--v-ink-2)] transition-colors hover:bg-[var(--v-surface-3)] hover:text-[var(--v-ink)]"
        >
          {vista === "menu" ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
        </button>
        {/*
          A identidade vive AQUI, ao lado dos traços — e só aqui. Repetir o
          nome num título grande abaixo da capa era a duplicação de antes; o
          logo, quando existe, substitui o texto pela mesma razão.
        */}
        {logoUrl ? (
          <img src={logoUrl} alt={emp.name} className="h-8 w-auto opacity-95" />
        ) : (
      ...
      {/*
        Barra de volta — só DENTRO de uma categoria. Na gaveta ela seria uma
        segunda barra dizendo o que o ✕ do cabeçalho já faz; no resumo, um
        cabeçalho vazio roubando altura da capa.
      */}
      {vista !== "resumo" && vista !== "menu" && (
      ...
      <ScrollArea className="flex-1 bg-[var(--v-bg)]" ref={detailRef}>
        <div className="px-4 py-4 space-y-3">
          ...
          {/* Ficha técnica no começo: são os números duros (terreno, torres,
              pé-direito, unidades) e é o que se procura logo depois do pitch. */}
          {FICHA_TECNICA}
          ...
          {/* ===== RESUMO: o que abre o painel =====
              Capa, nome, sobre e destaques -- o que convence alguem a
              continuar olhando. O resto virou categoria: antes tudo vinha
              numa rolagem unica, e o que ficava embaixo nunca era visto. */}
          {vista === "resumo" && (
            <>
          {destaques.length > 0 && (
            <div>
            ...
            <div className="v-in">
              {([
                ...
                },
                {
                  ...
                  icone: <Images className="w-4 h-4" />,
                ...
                .map((c) => (
                  <button
                    ...
                    className="flex w-full items-center gap-3 border-b border-[var(--v-line)] px-1 py-4 text-left transition-colors hover:bg-[var(--v-surface-2)]"
                  >
                    <span className="text-[var(--v-accent)]">{c.icone}</span>
                    <span className="flex-1 text-[15px] text-[var(--v-ink)]">{c.rotulo}</span>
                    {c.n != null && (
                      <span className="v-chip bg-[var(--v-surface-3)] text-[var(--v-ink-2)]">{c.n}</span>
                    )}
                    <ChevronRight className="w-4 h-4 text-[var(--v-ink-3)]" />
                  </button>
                ))}
            </div>
          )}
...
export default function EmpreendimentoPanel({
  ...
  return (
    <div
      data-testid="panel-empreendimentos"
      className={`absolute top-0 left-0 bottom-0 w-[460px] max-w-[94vw] z-30 flex flex-col glassmorphism transition-transform duration-300 ease-out ${
        isOpen ? "translate-x-0" : "-translate-x-full"
      }`}
