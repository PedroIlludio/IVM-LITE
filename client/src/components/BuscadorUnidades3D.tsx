...
import FaixaSlider from "@/components/FaixaSlider";
...
export default function BuscadorUnidades3D({
  ...
  const limites = useMemo(() => faixasDe(unidades), [unidades]);
        ...
        <div className="space-y-4 px-6 pb-5">
          ...
          {(limites.area || limites.preco) && (
            <div className="space-y-3.5">
              {limites.area && fArea && (
                <FaixaSlider label="Área" min={limites.area[0]} max={limites.area[1]} step={1}
                  value={fArea} onChange={setFArea} format={formatArea} />
              )}
              {limites.preco && fPreco && (
                <FaixaSlider label="Preço" min={limites.preco[0]} max={limites.preco[1]} step={1000}
          ...
          {avancados && (
            <div className="v-in space-y-3.5 border-t border-[var(--v-line)] pt-4">
              {limites.quartos && fQuartos && (
                <FaixaSlider label="Quartos" min={limites.quartos[0]} max={limites.quartos[1]} step={1}
