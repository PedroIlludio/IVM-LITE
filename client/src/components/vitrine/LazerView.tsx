...
export default function LazerView({ itens, onFechar }: {
  ...
  const portalPanorama = panorama?.panoramaUrl && createPortal(
        <div
  ...
  /*
    CELULAR: página vertical. Em tela cheia com `cover`, uma foto horizontal
    num aparelho em pé perdia as laterais — o ambiente não aparecia. Aqui a
    mídia fica inteira, na horizontal, como na galeria, e o texto vem abaixo.
  */
  if (movel) {
    return (
      <div className="absolute inset-0 z-20 bg-[#101410]" data-testid="lazer-view">
        <div className="vd-scroll h-full"
            ...
            {visiveis.length > 1 && (
              <>
                <button type="button" onClick={() => ir(-1)} aria-label="Ambiente anterior"
                  className="vd-seta absolute left-2 top-1/2 !h-11 !w-11 -translate-y-1/2">
                  <ChevronLeft className="h-5 w-5" strokeWidth={1.5} />
                </button>
                <button type="button" onClick={() => ir(1)} aria-label="Próximo ambiente"
                  className="vd-seta absolute right-2 top-1/2 !h-11 !w-11 -translate-y-1/2">
          ...
          <section className="px-4 pt-5" aria-live="polite">
